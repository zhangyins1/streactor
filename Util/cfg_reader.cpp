...
string CCfgReader::GetValue(const char* szSection, const char* szName)
{
    ...
    _LOG_INFO("CCfgReader::GetValue", "Empty item value: [%s] %s!", szSection, szName);
...
void CCfgReader::ScanCfgFile()
{
    ...
    if(!file.is_open())
    {
        _LOG_INFO("CCfgReader::ScanCfgFile",  "open config file failed: %s", m_strFile.c_str());
    }
    ...
    while(!file.eof() && file.getline(szLine, sizeof(szLine), 0x0A))
    {
        ...
        if(strSection.empty())
        {
            _LOG_INFO("CCfgReader::ScanCfgFile", "item '%s' has not section", sLine.c_str());
        }
